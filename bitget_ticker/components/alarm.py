...
class AlarmEngine:
    ...
    def __init__(
        self,
        on_alarm: Callable[[float, float, str], None],
    ...
    def check(self, price: float, alarms: Iterable[float | dict[str, object]]) -> None:
        ...
        active_keys = {key for key, _alarm_price, _mode in alarm_list}
        stale_keys = set(self.alarm_states) - active_keys
        for key in stale_keys:
            self.alarm_states.pop(key, None)

        for key, alarm_price, mode in alarm_list:
            ...
            self.alarm_states[key] = current_side
            self.on_alarm(alarm_price, price, mode)
            if mode == "popup":
    ...
    def _normalize_alarms(
        ...
        for index, alarm in enumerate(alarms):
            ...
            if isinstance(alarm, dict):
                ...
                raw_mode = alarm.get("mode", "popup")
                mode = raw_mode if raw_mode in {"popup", "notification"} else "popup"
            ...
            normalized.append((f"{index}:{alarm_price}:{mode}", alarm_price, mode))
