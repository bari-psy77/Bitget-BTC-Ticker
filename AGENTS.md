...
### 2.1 Superpowers MCP 와 로컬 Skills 구분
- `superpowers` MCP에서 쓸 수 있는 스킬 이름은 반드시 `mcp__superpowers__find_skills`에 나온 `superpowers:<name>`만 사용합니다.
- `.agent/skills`, `~/.codex/skills`, `~/.gemini/antigravity/skills`에 있는 로컬 스킬은 `superpowers` MCP와 별개입니다.
  - MCP 스킬: `mcp__superpowers__use_skill`로 호출
  - 로컬 스킬: 해당 경로의 `SKILL.md`를 직접 읽고 사용
- 로컬에는 있지만 MCP 목록에 없는 스킬을 `mcp__superpowers__use_skill`로 호출하면 `not found`가 정상입니다.
- 따라서 스킬을 찾을 때는 먼저 “이 스킬이 MCP 목록에 있는가”를 확인하고, 없으면 로컬 스킬 경로에서 찾습니다.
